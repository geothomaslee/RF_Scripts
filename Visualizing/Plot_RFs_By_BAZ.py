...
def Plot_Station_By_BAZ(stream):
    ...
    for trace in stream:
        ...
        times = trace.times()
        data = (trace.data * true_scale_factor) + baz
        ax.plot(data, times, "k-", linewidth=0.15)
        ax.fill_betweenx(times, data, baz,
...
def Plot_Station_By_False_BAZ(stream):
